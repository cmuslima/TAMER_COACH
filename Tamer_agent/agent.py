...
def step(env, reward:str):
    ...
    envState = {'observation': state, 'done': done, 'action': myagent.current_action, 'weights': myagent.w, 'current_budget_used': myagent.budget_counter}
    
    if (state[0] >= .5) or (myagent.time_step==500):
        done = True
        envState = {'observation': state, 'done': done, 'action': myagent.current_action, 'weights': myagent.w, 'current_budget_used': myagent.budget_counter, 'budget_per_episode': myagent.budget_per_episode}
